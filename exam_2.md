...
* В расширении $F \subset F(x)$ элемент $x$ --- трансцендентный над $F$. Тут $x$ обязательно должен быть $x$ многочленовым $x$, а не элементом $F$ (контрпример:  рассмотрим расширение $\Q(\sqrt{2}) \supset \Q$, тогда многочлен $x^2-2$ имеет своим корнем $\sqrt{2}$ $\Then$ $\sqrt{2}$ алгебраический).
