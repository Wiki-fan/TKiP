...
% Ильинский
% Обозначим смежный класс многочлена \(g(x) \in F\) по идеалу $(f(x))$ как \(\bar{g}(x) \in K\). Тогда имеем: \(\bar{x} \in K\) --- корень многочлена \(f(x)\), т. к. \(f(\bar{x}) = \bar{f}(\bar{x}) = 0\).
\end{solution}
...
% \(\forall\) многочлена \(h(x) \in F[x] \ \bar{h}(x)\) --- образ при факторизации по идеалу \((f(x))\) --- совпадает с \(\bar{r}(x)\), где \(r(x)\) --- остаток от деления \(h(x)\) на \(f(x)\). 
