...
Степень расширения $[\Q(\sqrt{2}):\Q] = 2$, ибо мин. многочлен для элемента $\sqrt2$: \(x^2-2\) --- степени 2. Значит, $\Q(\sqrt{2}) = \sfrac{\Q}{(x^2-2)}$.
...
Другие не рассматриваются, поскольку 1 и ЛНЗ корни многочлена образуют базис в \(\Q(\sqrt2)\) как линейном пространстве над \(\Q\); 1 при этом должен перейти в 1, т. к. \(\Q\) сохраняется (№3-4.40).
...
% Пусть \(\phi\) --- авторморфизм. Тогда \(\phi(m_\gamma(\gamma)) = \phi(0) = 0 \Then 0 = a_n \phi(\gamma)^m+\dots+a_0 \Then \phi(\gamma)\) --- корень \(m_\gamma\) \(\Then\) сопряжён с \(\gamma\).
...
\i 
	%Кажется, решение не совсем правильное.
